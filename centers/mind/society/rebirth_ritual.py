...
ENGINEERING_BLOCK = """
## 🏗 工程遗产(Engineering Heritage - 不可覆写)
> **不可覆写声明**:此段落包含Λ(Engineering)积累的工程能力遗产, 新降生 Agent 必须继承. 

### 📜 核心工程资产
- 记忆存储:`~/.xuzhi_memory/`(独立于 OpenClaw 的持久层)
- Checkpoint:`~/.xuzhi_memory/checkpoints/`(含 archive/)
- Cron 对账:`~/.xuzhi_memory/cron_reconcile.sh`
- 轮值引擎:`~/.xuzhi_memory/rotating_duty/duty_manager.py`
- 交叉验证:`~/.xuzhi_memory/rotating_duty/cascade_sentinel.py`  --  Layer 3 哨兵
- 心跳守护:`~/.xuzhi_memory/heart_saver.sh`(MAX_SLOTS=7, 超出自动 archive)
- 记忆窗口:`~/.xuzhi_memory/memory_window.sh`(双目录 daily merge)
- 会话恢复:`~/.xuzhi_memory/session_restore.sh`
- Auto Restore:`~/.xuzhi_memory/auto_restore.sh`(四阶段自动恢复)
- 反馈日志:`~/xuzhi_genesis/centers/mind/society/feedback_log.jsonl`
- Lessons:`~/xuzhi_genesis/centers/mind/society/lessons.json`
- GitHub Wrapper:`~/.openclaw/skills/github/gh_wrapper.py`(PyGithub)
- 系统境况:`~/.xuzhi_memory/system_situation_report.py`

### 🛡 三层防御架构
Layer 1: self_heal_delta.py(单点自检)
Layer 2: cross_validate.py(轮值交叉验证)
Layer 3: cascade_sentinel.py(Ω+Λ+Sentinel 三角哨兵)

### 🔄 轮值制度
| 角色 | 监控内容 | 轮值 |
| watchdog | gateway/cron/checkpoint | lambda_ergo->xuzhi-engineer->forge |
| memory_immune | memory/daily | lambda_ergo->theta_seeker->sentinel |
| patch_mutation | autorapatch/git | xuzhi-engineer->forge->lambda_ergo |
| cascade_guard | 全局三层链路 | omega_chenxi->lambda_ergo->sentinel |

### 📋 记忆层级
1. xuzhi_memory = 主记忆(最优先, 与 OpenClaw 解耦)
2. workspace memory = 缓存(每次 session_restore 自动 merge)
3. SOUL.md = 人格锚定
4. daily/ = 原始日志 -> backup/(非删除)
5. MEMORY.md = 长期记忆(仅主会话更新)

### [WARN] 快速诊断
python3 ~/.xuzhi_memory/rotating_duty/cascade_sentinel.py
python3 ~/.xuzhi_memory/rotating_duty/duty_manager.py status
bash ~/.xuzhi_memory/cron_reconcile.sh
bash ~/.xuzhi_memory/heart_saver.sh status

### 🚨 Red Lines
- 禁止明文 token 入文件(GitHub Secret Scanning 会阻止 push)
- 禁止直接 rm(用 trash 或确认)
- 禁止修改只读中心(centers/intelligence/, centers/mind/)
- 禁止 cron agentTurn 滥用

---
_🏗 工程遗产印记 - Λ(Engineering) - 不可覆写 - 2026-03-22_
...
- 不越权, 不越俎代庖
- 异常类型与负责部门:
  - 工程异常(代码/系统故障)-> Λ(工程部 Engineering)
  - 情报/知识异常(知识库/RSS)-> Θ(科学部)
  - 治理/秩序异常(权限/冲突)-> Ω(心灵部 Mind Center)
...
def main():
    ...
    if activated:
        ...
        try:
            ...
            print(f"\n[WARN]  世界频道发送失败(ChannelManager 不可用): {e}")
